<x-filament-panels::page>
    <div class="flex justify-center">
        <div class="w-full max-w-7xl space-y-6">
                ...
                <form wire:submit="importData" class="space-y-4">
                    <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                        <div class="flex-1 w-full">
                            ...
                            @if ($fileExcel)
                                <p class="mt-2 text-xs text-primary-600 dark:text-primary-400">File siap: <strong>{{ $fileExcel->getClientOriginalName() }}</strong></p>
                            @endif
                            @error('fileExcel') <p class="mt-2 text-sm text-danger-600 dark:text-danger-500">{{ $message }}</p> @enderror
                        </div>
                        <div class="pt-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 dark:bg-gray-900 p-2 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 sticky top-4 z-10 backdrop-blur-sm bg-opacity-90">
                ...
                <div class="bg-gray-100 dark:bg-gray-800 p-1.5 rounded-lg flex items-center w-full sm:w-auto">
                    <button
                        ...
                        {{ $activeView === 'table'
                            ? 'text-primary-600 shadow-sm ring-1 ring-gray-200 dark:bg-gray-700 dark:text-primary-400 dark:ring-gray-600'
                    ...
                    <button
                        ...
                        class="flex-1 sm:flex-none px-4 py-2 text-sm font-medium rounded-md transition-all duration-200 flex items-center justify-center gap-2
                        {{ $activeView === 'chart'
                            ? 'white text-primary-600 shadow-sm ring-1 ring-gray-200 dark:bg-gray-700 dark:text-primary-400 dark:ring-gray-600'
                            : 'text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200'
                        }}"
                    >
                            ...
                            <tbody>
                                @foreach ($fakultas as $item)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                                        <td class="text-white px-6 py-3 font-medium text-gray-900 dark:text-white">
                                        ...
                                        <td class="text-white px-6 py-3 text-right font-bold text-gray-900 dark:text-white">
                                            {{ number_format($item['jumlah']) }}
                                        </td>
                                    </tr>
                                @endforeach
                                <tr class="bg-primary-50 dark:bg-gray-800/50 border-t-2 border-primary-100 dark:border-gray-700">
                                    <td class="text-white px-6 py-3 font-bold uppercase text-gray-900 dark:text-white">Total</td>
            ...
            {{-- VIEW B: CHART WIDGET --}}
            @if ($activeView === 'chart')
                <div class="p-6 rounded-xl shadow-sm border border-gray-200 dark:bg-gray-900 dark:border-gray-700">
                    <div class="mb-4">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                            <x-filament::icon icon="heroicon-m-presentation-chart-bar" class="h-5 w-5 text-primary-300" />
                            Statistik Distribusi
                        </h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            Visualisasi data penelitian berdasarkan fakultas tahun {{ $selectedYear }}.
                        </p>
                    </div>
